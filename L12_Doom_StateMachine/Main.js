...
(function (L12_Doom_StateMachine) {
    ...
    L12_Doom_StateMachine.avatar = new f.Node("Avatar");
    ...
    async function hndLoad(_event) {
        ...
        floor.mtxLocal.scale(f.Vector3.ONE(L12_Doom_StateMachine.sizeWall * L12_Doom_StateMachine.numWalls));
        floor.getComponent(f.ComponentMaterial).pivot.scale(f.Vector2.ONE(L12_Doom_StateMachine.numWalls));
        ...
        L12_Doom_StateMachine.avatar.addComponent(new f.ComponentTransform());
        L12_Doom_StateMachine.avatar.mtxLocal.translate(f.Vector3.Z(10));
        L12_Doom_StateMachine.avatar.mtxLocal.rotate(f.Vector3.Y(180));
        root.appendChild(L12_Doom_StateMachine.avatar);
        L12_Doom_StateMachine.viewport = new f.Viewport();
    ...
    async function createEnemies() {
        ...
        for (let i = 0; i < 1; i++)
            enemies.appendChild(new L12_Doom_StateMachine.Enemy("Cacodemon" + i, f.Vector3.Z(3)));
    ...
    function createWalls() {
        ...
        walls.appendChild(new L12_Doom_StateMachine.Wall(f.Vector2.ONE(3), f.Vector3.Y(L12_Doom_StateMachine.sizeWall / 2), f.Vector3.ZERO(), mtrWall));
        walls.appendChild(new L12_Doom_StateMachine.Wall(f.Vector2.ONE(3), f.Vector3.SCALE(new f.Vector3(0.5, 1, -0.866), L12_Doom_StateMachine.sizeWall / 2), f.Vector3.Y(120), mtrWall));
        walls.appendChild(new L12_Doom_StateMachine.Wall(f.Vector2.ONE(3), f.Vector3.SCALE(new f.Vector3(-0.5, 1, -0.866), L12_Doom_StateMachine.sizeWall / 2), f.Vector3.Y(-120), mtrWall));
        for (let i = -L12_Doom_StateMachine.numWalls / 2 + 0.5; i < L12_Doom_StateMachine.numWalls / 2; i++) {
            walls.appendChild(new L12_Doom_StateMachine.Wall(f.Vector2.ONE(3), f.Vector3.SCALE(new f.Vector3(-L12_Doom_StateMachine.numWalls / 2, 0.5, i), L12_Doom_StateMachine.sizeWall), f.Vector3.Y(90), mtrWall));
            // for (let i: number = -numWalls / 2 + 0.5; i < numWalls / 2; i++)
            walls.appendChild(new L12_Doom_StateMachine.Wall(f.Vector2.ONE(3), f.Vector3.SCALE(new f.Vector3(L12_Doom_StateMachine.numWalls / 2, 0.5, i), L12_Doom_StateMachine.sizeWall), f.Vector3.Y(-90), mtrWall));
            // for (let i: number = -numWalls / 2 + 0.5; i < numWalls / 2; i++)
            walls.appendChild(new L12_Doom_StateMachine.Wall(f.Vector2.ONE(3), f.Vector3.SCALE(new f.Vector3(i, 0.5, -L12_Doom_StateMachine.numWalls / 2), L12_Doom_StateMachine.sizeWall), f.Vector3.Y(0), mtrWall));
            // for (let i: number = -numWalls / 2 + 0.5; i < numWalls / 2; i++)
            walls.appendChild(new L12_Doom_StateMachine.Wall(f.Vector2.ONE(3), f.Vector3.SCALE(new f.Vector3(i, 0.5, L12_Doom_StateMachine.numWalls / 2), L12_Doom_StateMachine.sizeWall), f.Vector3.Y(180), mtrWall));
        }
        return walls;
    }
